...
class Logger(object):
    '''
    Logger object for tensorboard
    '''
    def __init__(self, logdir: str=None, summary_file='summary.csv'):
    ...
    def flush(self) -> None:
        ...
        for tag in self.log_data:
            # tag_file = os.path.join(self.log_dir, "{}.csv".format(tag))

            # create the output tag file if it does not
            # exist
            # csv_file = utils.get_or_create_file(tag_file)
            
            # write all the data in csv format to the
            # tag file
            # writer = csv.writer(csv_file)
            for (value, step) in self.log_data[tag]:
                # writer.writerow([value, step])
