...
class BiLSTM_CRF(nn.Module):
    ...
    def __init__(self, vocab_size, tag_to_ix, embedding_dim, hidden_dim):
        ...
        self.transitions.data[tag_to_ix[constants.START_TOKEN], :] = -10000
        self.transitions.data[:, tag_to_ix[constants.END_TOKEN]] = -10000
    ...
    def _forward_alg(self, feats):
        ...
        init_alphas[0][self.tag_to_ix[constants.START_TOKEN]] = 0.
        ...
        terminal_var = forward_var + self.transitions[self.tag_to_ix[constants.END_TOKEN]]
        alpha = log_sum_exp(terminal_var)
        return alpha

    def _get_lstm_features(self, sentence):
        sentence = sentence[0].long()
        self.hidden = self.init_hidden()
    ...
    def _score_sentence(self, feats, tags):
        ...
        tags = tags[0].long()
        score = torch.zeros(1)
        tags = torch.cat([torch.tensor([self.tag_to_ix[constants.START_TOKEN]], dtype=torch.long), tags])
        for i, feat in enumerate(feats):
            score = score + \
                self.transitions[tags[i + 1], tags[i]] + feat[tags[i + 1]]
        score = score + self.transitions[self.tag_to_ix[constants.END_TOKEN], tags[-1]]
        return score
    ...
    def _viterbi_decode(self, feats):
        ...
        init_vvars[0][self.tag_to_ix[constants.START_TOKEN]] = 0
        ...
        start = best_path.pop()
        assert start == self.tag_to_ix[constants.START_TOKEN]  # Sanity check
        best_path.reverse()
    ...
    def forward(self, sentence):  # dont confuse this with _forward_alg above.
        ...
        return torch.Tensor(tag_seq), score
