...
class LoadModelButton extends React.Component {
  state = {
      is_running: false,
  };
  ...
  handleSaveClick() {
    ...
    fetch(
        get_user_url(`${ACTIVE_LEARNING_SERVER}/api/load_model/`)
    ).then(results => (
        results.json()
    )).then(results => {
...
LoadModelButton.propTypes = {
  classes: PropTypes.object.isRequired,
  buttonProps: PropTypes.object,
};

export default withRoot(withStyles(styles)(LoadModelButton));
